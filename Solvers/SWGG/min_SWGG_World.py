...
class min_SWGG_World(Defense_Train_Base):
    def __init__(
        ...
        # Precompute fixed geodesic cost matrix (for Sinkhorn comparison)
    ...
    def predict_plan(self, a: np.ndarray, b: np.ndarray) -> np.ndarray:
        ...
        # ── Sample L random unit directions in R^3 ────────────────────────
        # theta: (3, L)
        thetas = torch.randn(3, L, dtype=torch.float64, device=device)
        thetas = thetas / thetas.norm(dim=0, keepdim=True).clamp(1e-12)

        # ── SWGG for all L directions ─────────────────────────────────────
        # quantile_SWGG_CP handles X ≠ Y and n_s ≠ n_t via quantile coupling
        with torch.no_grad():
        ...
        # u_w[k] = original supply index for quantile piece k (at θ*)
        # v_w[k] = original demand index for quantile piece k (at θ*)
        # dr[k]  = mass of piece k
        ...
        # Filter tiny/negative mass (numerical noise)
        mask  = dr > 1e-15
        u_w   = u_w[mask]
        v_w   = v_w[mask]
        dr    = dr[mask]
    ...
    def train(self, dataloader_train, dataloader_test=None):
        ...
        Optionally runs a quick sanity check if dataloader_test is given.
        """
        self.logger.info(
