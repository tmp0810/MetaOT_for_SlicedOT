...
class AmortizedOA_OT_CIFAR:
    def __init__(self, L: int = 100, eps: float = 800.0,
        ...
        # proj_dirs stored as float64 (generated by library) — will be cast
        # to float32 on demand for fast GPU matmul.
    ...
    def _flatten(self, x: torch.Tensor) -> torch.Tensor:
        """(B, C, H, W) -> (B, D) float64 on self.device (kept for pretrain)."""
    ...
    def _compute_sliced_potentials_f32(self, x0_flat: torch.Tensor,
        ...
        B   = x0_flat.shape[0]
        dev = x0_flat.device
        # float32 GPU matmul → CPU → upcast to f64 only for emd1D_dual
    ...
    def _g_from_f(f: torch.Tensor, b: torch.Tensor,
        ...
        # clamp to float32-safe floor (1e-38 ≈ float32 min normal)
    ...
    def pretrain(self, source_sampler, target_sampler,
        ...
        # ── Phase 1b: optimise alpha via dual objective ───────────────────────
        ...
        # store as numpy float32 array (matches predict_plan expectations)
    ...
    def predict_plan(self, x0: torch.Tensor, x1: torch.Tensor) -> torch.Tensor:
        ...
        alpha_t = torch.tensor(self.alpha, dtype=torch.float32)           # (L,) CPU f32
    ...
    def predict_plan_cpu(self, x0: torch.Tensor, x1: torch.Tensor) -> torch.Tensor:
        ...
        # ── 2. Sliced potentials (f32 path, emd1D_dual on CPU) ────────────
