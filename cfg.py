...
def init_cfg(n_solver):
    ...
    if n_solver == "OT_Discrete":
        ...
        cfg_m.insert("epsilon", 0.001)          # entropic regularisation ε — must be large enough so K=exp(-C/ε) has no zero entries (ε=0.01 causes 57% of K to be ~0 → Sinkhorn diverges)
