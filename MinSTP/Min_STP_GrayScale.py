...
class SlicerNet(nn.Module):
    """
    Amortized slicer for MNIST.

    Architecture (following amortized_slicer.py spirit):
      Context encoder : MLP( concat(a, b) ) → c  (float64 throughout)
      Per-point head  : MLP( [x_i, c] )     → s_i scalar score
    """
    ...
    def __init__(self, n_pixels: int = 784,
                 ...
                 hidden_dim: int = 128):
        super().__init__()
        # Context: (a, b) concatenated → small vector
        ...
        # Per-point: [x_i (coord_dim), c (context_dim)] → scalar score
    ...
    def forward(self, a: torch.Tensor, b: torch.Tensor,
        ...
        """
        a, b   : (n,)   float64 weight vectors
        x_grid : (n, 2) float64 pixel coordinates
        Returns: scores (n,) float64
        """
...
class Min_STP_GrayScale(Defense_Train_Base):
    """
    Amortized Min-STP baseline for grayscale (MNIST) experiments.

    Training  : minimise <P(slicer(a,b)), C>  via soft_permutation + hard_sort
                (two-branch symmetric plan, following min-STP paper)
    Inference : exact weighted 1-D OT via quantile matching with learned scores
                → ambient transport plan P ∈ ℝ^{n×n}
    """
    ...
    def train(self, dataloader_train):
        """
        Amortized training loop.
        For each gradient step: sample a random train pair (a,b),
        compute slicer scores, build symmetric soft/hard plan, compute
        ambient cost <P, C>, backprop.
        """
        cfg    = self.cfg_m
        ...
        for step in range(T):
            ...
            # ── two-branch symmetric plan (eq. 5 of min-STP paper) ────────────
            # Branch 1: soft-perm(X) ᵀ @ hard-sort(Y)
        ...
        # Save slicer weights alongside checkpoint
    ...
    def predict_plan(self, a: np.ndarray, b: np.ndarray) -> np.ndarray:
        """
        Inference: use learned per-point scores with exact weighted 1-D OT.

        1. Forward slicer → per-point scores s ∈ ℝ^{784}
        2. Sort support points by s → compute weighted CDF for a and b
        3. Quantile-match CDFs → sparse plan P
        4. Return P as (784, 784) numpy array
        """
        device = self.device
        ...
        # Sort both source and target by the SAME scores
        u = torch.argsort(scores)   # source sort order
        v = torch.argsort(scores)   # target sort order (same grid)
        ...
        # Merge quantile levels (union of CDF breakpoints, excluding last 1.0)
        r_all, _ = torch.sort(torch.cat([rA, rB]))
        r_all    = r_all[:-1]                  # remove trailing 1.0

        # Active segment indices for source and target
        ...
        # Filter near-zero intervals
        mask    = delta_r > 1e-15
        wA      = wA[mask]
        wB      = wB[mask]
        delta_r = delta_r[mask]

        # Map back to original (pre-sort) indices
