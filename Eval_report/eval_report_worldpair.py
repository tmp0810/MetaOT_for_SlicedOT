...
def main():
    ...
    train_pairs_list = [sample_pair(args.n_supply, args.n_demand,
                                    TRAIN_SEED + i) for i in range(args.M)]
    dl_shared = pairs_to_world_loader(train_pairs_list, batch_size=1)
    print(f"  {len(train_pairs_list)} train pairs ready.\n")

    results = []

    def make_loader(cfg_m, seed):
    ...
    model3 = Meta_OT_World(
        make_cfg_proj("Meta_OT_World", TRAIN_SEED, args.gpu, flag_time),
    ...
    model4 = min_SWGG_World(
        make_cfg_proj("min_SWGG_World", TRAIN_SEED, args.gpu, flag_time),
